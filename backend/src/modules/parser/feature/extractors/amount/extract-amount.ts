...
export function stripDateExpressions(input: string): string {
  return input
    .replace(PATTERN_ISO_DATE, " ")
    .replace(PATTERN_US_DATE, " ")
    .replace(PATTERN_NAMED_MONTH_DATE, " ")
    .replace(PATTERN_SIMPLE_RELATIVE, " ")
    .replace(PATTERN_LAST_WEEKDAY, " ");
}
...
export function extractAmount(normalizedInput: string): AmountResult {
  ...
  const bareSafeInput = stripDateExpressions(normalizedInput);
